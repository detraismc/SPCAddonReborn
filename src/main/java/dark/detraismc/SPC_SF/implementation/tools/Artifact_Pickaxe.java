...
public class Artifact_Pickaxe extends SimpleSlimefunItem<ToolUseHandler> {
    ...
    public ToolUseHandler getItemHandler() {
        return (e, tool, fortune, drops) -> {
            // 1. Check if the event is already cancelled before doing anything
            if (e.isCancelled()) {
                return;
            }
            ...
            if (b.getType() == Material.END_STONE) {
                // 2. Prevent the default Vanilla End Stone from dropping safely
                e.setDropItems(false);

                if (plugin.mtk.getChance(12, 100)) {
                    if (plugin.mtk.getChance(30, 100)) {
                        ...
                        // Fixed: These were previously GOLDEN_CHESTPLATE
                    ...
                    // 3. Drop the Slimefun item naturally into the world instead of modifying the drops list
                    b.getWorld().dropItemNaturally(b.getLocation(), SlimefunItems.STONE_CHUNK);
                }
            }
        };
    }
